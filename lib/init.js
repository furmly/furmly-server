...
const furmlyEngine = require("./setup_furmly_engine");
...
function _getIconForDefaultProcess(proc) {
  ...
  if (title.indexOf("process") !== -1) return "laptop";
  if (title.indexOf("client" !== -1)) return "mobile";
}
        ...
        infrastructure.saveMenu(
          {
            ...
            value: proc._id,
            category: "MAINMENU",
            client: infrastructure.studioClient._id,
            activated: true
          },
          callback
        );
  ...
  infrastructure.init(
    ...
    function(er) {
      ...
      furmlyEngine.init(function(er) {
        ...
        debug("starting threadpool....");
        threadPool.start();
        //create default processors.
        debug("setting up server defaults...");
        setup(furmlyEngine, onProcessCreated, onProcessorCreated, er => {
          if (!er) return debug("defaults setup properly");
          setTimeout(() => {
            debug(er);
            throw er;
          }, 0);
