...
module.exports = function(config) {
	...
	this.parseOnly = (description, data, fn) => {
		...
		if (!parser || !parser.parseOnly)
			return fn(new Error("Cannot Parse a file of that type"));

		parser.parseOnly(description, data, fn);
	};
	this.generate = (fileType, data, fn) => {
		let parser = this.getParser({ originalName: `.${fileType}` }, data);
		if (!parser || !parser.generate)
			return fn(new Error("Cannot generate a file of that type"));
