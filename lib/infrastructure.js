const localUtils = require("./utilities"),
	...
	async = require("async"),
	debug = require("debug")("infrastructure"),
...
function Infrastructure(opts) {
	...
	if (!opts.userStore)
		throw new Error("Infrastructure must have a user store");

	if (!opts.roleStore)
		throw new Error("Infrastructure requires a role store");
	...
	if (!opts.menuStore)
		throw new Error("Infrastructure requires a menu store");
...
Infrastructure.prototype.init = function(
	...
	function createDefaultRole(er, claims) {
		...
		self.roleStore.getRole(self.defaultRole, function(er, role) {
			...
			let difference = localUtils.difference(
				claims,
				role.claims,
				function(item, list) {
	...
	function createDefaultClaims() {
		...
		this.claimsStore.get(
			...
			function(er, res) {
				...
				debug(
					`discovered new claims ? ${JSON.stringify(
						claims,
						null,
						" "
					)}`
	...
	this.roleStore.getRole(name, domain, function(er, role) {
		...
		self.roleStore.update(
			role._id,
			{
				$push: {
					claims: {
						$each: (Array.prototype.isPrototypeOf(claims) &&
							claims) || [claims]
					}
				}
			},
...
Infrastructure.prototype.saveMigration = function(data, fn) {
	this.migrationStore.save(data, fn);
};
