...
function Infrastructure(opts) {
  ...
  this.webClient = opts.webClient || {
    _id: "U6Ev60tUEFw6LZYe3lttdWeG8VagXdJi",
    name: "Website",
    clientSecret: "V4JQ1f52RUTw4TrqLRcrdLNJhChkWDer"
  };
  this.studioClient = opts.studioClient || {
    _id: "eGuzz3SqVEvFgRU2SALS6AG5mQCuG4y5",
    name: "Studio",
    clientSecret: "Ut8ASzJjf8DBxNY34QKLUndJyPGzkyBc"
  };
  this.mobileClient = opts.mobileClient || {
    _id: "hDA4nGaIkHmdh3NtQmRewBkxIXNsyzyB",
    name: "Mobile App",
...
Infrastructure.prototype.init = function(
  ...
  function createDefaultClaims() {
    let claims = Object.keys(this.adminClaims).map(prop => {
    ...
    let fullClaims = claims.filter(x => typeof x == "object");
...
ranges.forEach(function(x) {
  ...
  Infrastructure.prototype[`count${x.className}`] = function() {
    this[x.storeName].count.apply(
      this[x.storeName],
      Array.prototype.slice.call(arguments)
    );
  };
...
function _createToken(user, client, domain, fn) {
  ...
  user.client = client;
  debug("compression enabled:" + this.config.compress);
  var encodedUser = this.config.compress
    ? zlib.deflateSync(JSON.stringify(user)).toString("base64")
    : user;
  debug("encoded user:" + encodedUser);
  var accessToken = this.tokenGen.sign(encodedUser, expTime);
...
Infrastructure.prototype.checkPassword = function(
  ...
  debug(`checking password for ${username}`);
  this.userStore.getUser(createUsernameQuery(username), domain, (er, user) => {
    if (er) return fn(er);

    if (!user) return fn(new Error("Invalid Credentials"));

    if (user.locked) {
      debug(`${username} is locked`);
      return fn(new Error("User Account is locked"));
    }
    ...
    bcrypt.compare(password, user.password, (er, res) => {
      ...
      if (!res) {
        ...
        if (user.passwordAttempts >= this.config.maxInvalidCredentials) {
          user.locked = true;
          debug(`${username} is now locked`);
        }

        this.userStore.save(user, false, er => {
          if (er) return fn(er);
          if (user.locked) return fn(new Error("User Account is locked"));
...
const createUsernameQuery = username => new RegExp(`^${username}$`, "i");
Infrastructure.prototype.unlockUser = function(username, domain, fn) {
  if (!username) return setImmediate(fn, new Error("username cannot be blank"));
  this.userStore.getUser(createUsernameQuery(username), domain, (er, user) => {
    if (er) return fn(er);
    if (!user) return fn(new Error("User does not exist"));

    user.locked = false;
    this.userStore.save(user, false, fn);
  });
};
...
Infrastructure.prototype.register = function(data, fn) {
  ...
  this.userStore.exists(
    createUsernameQuery(data.username),
    data.domain,
    function(er, exists) {
      if (er) return fn(er);
  ...
  try {
    ...
    if (this.config.compress)
      result.data = JSON.parse(
        zlib.unzipSync(Buffer.from(result.data, "base64")).toString()
      );
    fn(null, result);
  } catch (e) {
    debug("an error occurred while verifying");
    debug(e);
    fn(e);
  }
};
...
/**
 * Passthrough method for clientStore.get
 * @param {Object} query Search criteria
 * @param {function} fn Callback
 */
Infrastructure.prototype.getClients = function(query, fn) {
  this.clientStore.get(query, fn);
};

/**
 * Passthrough method for clientStore.saveClaim
 * @param  {Object}   data client to save
 * @param  {Function} fn    Callback
 * @return {Object}         Saved Object.
 */
Infrastructure.prototype.saveClient = function(data, fn) {
  if (!data.clientSecret)
    return setImmediate(fn, new Error("All clients must have a secret"));
  if (!data._id)
    return setImmediate(fn, new Error("All clients must have an _id"));
  this.clientStore.save(data, fn);
};
