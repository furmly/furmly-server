...
Store.prototype.save = function(data, fn) {
...
Store.prototype.get = function(query, fn) {
	this.model
		.find(query)
		.lean()
		.exec(fn);
...
Store.prototype.createOrUpdate = function(key, data, fn) {
	this.model.update(
		key,
		data,
		{
...
Store.prototype.getRange = function(query, count, fn) {
	var self = this;
...
Store.prototype.delete = function(query, fn) {
	this.model.remove(query).exec(fn);
};

function getRangeAndTransform(query, count, fn) {
	var self = this;
	Store.prototype.getRange.call(this, query, count, function(er, result) {
		if (er) return fn(er);
...
function MongoStore(name, schema, mongoose, conn) {
	...
	Store.call(this, name, conn.model(name, new mongoose.Schema(schema)));
}
