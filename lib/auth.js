...
function User(opts) {
	...
	if (!opts.roles || !opts.roles.length) throw new Error("Users must have atleast one role");
...
function UserManager(opts) {
	...
	if (!opts.claimsStore) throw new Error("UserManager requires a claims store");

	if (!opts.tokenGen) throw new Error("UserManager requires a token generation strategy");

	if (!opts.menuStore) throw new Error("UserManager requires a menu store");

	if (!opts.clientStore) throw new Error("UserManager requires a clientStore");
	if (!opts.domainStore) throw new Error("UserManager requires a domainStore");
			...
			function(er, result) {
				...
				if (result.length !== menus.length) {
					result.forEach(function(x) {
						if (menus.indexOf(x.uid) !== -1) menus.splice(menus.indexOf(x.uid), 1);
					});
...
function _createToken(user, client, domain, fn) {
	...
	this.userStore.createRefreshToken(refreshTokenHash, user.username, client.clientId, domain, function(er, result) {
 ...
 * @return {Object}                  token
 */
UserManager.prototype.refreshToken = function(domain, client, refreshToken, fn) {
	let self = this,
	...
	this.userStore.getRefreshToken(refreshTokenHash, client.clientId, domain, function(er, token) {
		if (er) return fn(er);
...
UserManager.prototype.checkPassword = function(domain, username, password, fn) {
	...
	this.userStore.getUser(new RegExp(`^${username}$`, "i"), domain, function(er, user) {
		if (er) return fn(er);
...
UserManager.prototype.changePassword = function(username, domain, password, newPassword, fn) {
...
UserManager.prototype.register = function(data, fn) {
	var self = this;
	debug(data);
	this.userStore.exists(new RegExp(`^${data.username}$`, "i"), data.domain, function(er, exists) {
		if (er) return fn(er);
 ...
 */
UserManager.prototype.acl = function(username, domain, client, category = "MAINMENU", fn) {
	var self = this;
...
UserManager.prototype.saveClaim = function(data, fn) {
	if (!data.type) return fn(new Error("All claims must have a type"));
...
function MongoStore(name, schema, mongoose, conn) {
	if (!mongoose || !conn || !name || !schema) throw new Error("mongoose is required by MongoStore");
...
MongoStore.prototype.get = function(query, fn) {
	this.model.find(query).lean().exec(fn);
};
...
function MongoDBUserStore(mongoose, conn) {
	if (!mongoose || !conn) throw new Error("user store requires an open mongoose connection");
 ...
 */
MongoDBUserStore.prototype.createRefreshToken = function(refreshTokenHash, username, clientId, domain, fn) {
	let col = this.connection.db.collection("refreshTokens");
	col.insertOne(
		{
 ...
 */
MongoDBUserStore.prototype.getRefreshToken = function(refreshTokenHash, clientId, domain, fn) {
...
function MongoDBRoleStore(mongoose, conn) {
	if (!mongoose || !conn) throw new Error("role store requires an open mongoose connection");
	var Schema = mongoose.Schema;
...
MongoDBRoleStore.prototype.getRoles = function(query, options, fn) {
	if (Array.prototype.slice.call(arguments).length == 2 || (!fn && Function.prototype.isPrototypeOf(options))) {
		fn = options;
		options = null;
	}
...
function MongoMenuStore(mongoose, conn) {
	if (!mongoose || !conn) throw new Error("mongoose is required by MongoMenuStore");
