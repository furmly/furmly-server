...
const fs = require("fs"),
  ...
  debug = require("debug")("furmly-server:file-upload"),
  path = require("path");
/**
 * File Upload
 * @classS
...
function FileUpload(config, mongoose, conn) {
  ...
  let MongoStore = require("../stores/store");
  this.MIMES = {
      ...
      timeout = setTimeout(() => {
        ...
        fs.unlink(fileInfo.path, er => {
          ...
          this.store.delete({ _id: saved._id }, err => {
            if (err) debug(`Error occured while deleting ${saved._id}`);
          });
        });
      }, this.config.ttl);
 ...
 * @param {Boolean} removeUserContext Users to determine the access level for this file
 * @param  {Function} fn Callback
 *
 */
FileUpload.prototype.moveToPermanentSite = function(id, removeUserContext, fn) {
  if (Array.prototype.slice.call(arguments).length == 2) {
    fn = removeUserContext;
    removeUserContext = false;
  }
  this._getFile(id, (er, item) => {
    if (er) return fn(er);
    let cancelPending = () => {
      let cancel = this.pending.filter(x => x._id.toString() == id.toString());
      if (cancel.length) {
        try {
          //cancel timeout
    ...
    fs.rename(item.path, newPath, er => {
      ...
      cancelPending();
      //update path.
      let update = { _id: id, path: newPath, isPerm: true };
      if (removeUserContext) update.user = null;
      this.store.save(update, fn);
    });
  });
};
/**
